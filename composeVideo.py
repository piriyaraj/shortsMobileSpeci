...
def addTexts(title,data)->None:
    ...
    for i in range(len(keys)): 
        ...
        draw.text(titlePosition,keys[i],(255,255,0),font=titleFont)

        # model name update
        modelSize=titleFont.getsize(title)
        titlePosition=(int((imageSize[0]-modelSize[0])/2),0)
        draw.text(titlePosition,title,(255,0,0),font=titleFont)
        ...
        for key,value in data.get(keys[i]).items():
            ...
            draw = ImageDraw.Draw(templateTemp)
            width, height = titleFont.getsize(key)

            draw.text((40, y_text), "->"+key,(31,206,203),font=titleFont)
            y_text += height+10

            lines = textwrap.wrap(value, width=35)
            for line in lines:
                width, height = titleFont.getsize(line)

                draw.text((80, y_text), line,(255,255,0),font=titleFont)
                y_text += height+10
...
def makeVideo(title)->None:
    postClips=[]
    size = (1080,1920)
    tempImages=os.listdir("videoImg")
    for i in tempImages:
        absdir=os.path.abspath("videoImg/"+i)
        postClips.append(ImageClip(absdir).set_duration(6).resize(size))
